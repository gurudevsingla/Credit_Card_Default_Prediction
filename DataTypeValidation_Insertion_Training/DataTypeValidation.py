...
class dBOperation:
    ...
    def __init__(self):
        ...
        self.db_columns_path = "training_columns_dbwise.json"
    ...
    def createTableDb(self, DatabaseName, column_names):
        ...
        try:
            ...
            session.execute('DROP TABLE IF EXISTS Good_Raw_Data').one()
            for key in list(column_names.keys()):
                type = column_names[key]

                # in try block we check if the table exists, if yes then add columns to the table
                # else in catch block we will create the table
                try:
                    session.execute("ALTER TABLE Good_Raw_Data ADD {column_name} {dataType}".format(column_name=key.replace('.','_'),dataType=type)).one()
                except:
                    row = session.execute("CREATE TABLE IF NOT EXISTS Good_Raw_Data ({column_name} {dataType} primary key)".format(column_name=key, dataType=type)).one()
            ...
            try:
                with open(goodFilePath+'/'+file, "r") as f:
                    ...
                    for line in enumerate(reader):
                        for list_ in (line[1]):
                            try:
                                session.execute('INSERT INTO Good_Raw_Data ({attributes}) values ({values})'.format(attributes = (", ".join(columns_list).replace('.','_')),values=(list_))).one()
    ...
    def selectingDatafromtableintocsv(self, Database):
        ...
        try:
            ...
            session = self.dataBaseConnection(Database)
            cassandraSelect = "SELECT *  FROM Good_Raw_Data"
